...
- **Resolution is by confidence (D1, added 2026-08-26).** `install_methods` ordered the ladder by how cheap each attempt was — it said so: *"ORDER IS COST, NOT CONFIDENCE"* — and gated every rung but the first on a hand-written dictionary. A technology in neither dictionary therefore had a one-rung ladder: guess the package name. RabbitMQ sat uncertified behind that sentence for days and .NET spent five machines on it.
  ...
  **The container is not preferred, and that was a correction.** D1 first inverted the order to put containers first, on the argument that a container requires no guessing. The tests it would have broken carried the answer: *"a container is the last resort, not the first — it changes how the service is patched, backed up and monitored"*, and *"an OS package is one command and no trust granted; a vendor repository trusts a publisher for everything it will ever serve."* Those are operational judgements about how the estate is run, and no failure was ever caused by the order — every one was caused by guessing the package name. The install order therefore stands; what changed is that the name is looked up, and that a container becomes reachable once the repositories have positively said they do not carry the software, which previously cost a machine to establish.

  The trust boundary for that container is narrow and deliberate: an allowed registry, pinned by digest, and only an image the registry calls official or one published under the technology's own vendor namespace. Popularity is not provenance. The allow-list was broadened to the major first-party publishers on the reviewer's instruction and is extensible from the Admin console (`CONTAINER_REGISTRIES`), because a vendor nobody anticipated should be one setting away rather than one release away.
...
10. **Where placement entitlement is enforced** — ✅ **RESOLVED 2026-09-08: the API is the authority; the BFF filters for display only.** The placement task specified entitlement filtering in the BFF. The BFF is the right place to shape what a browser is shown, but it cannot be the control: the API listens on its own port, so anything checked only in the BFF is bypassable by anyone who can reach it — which contradicts P1 and P2. `api/clusters.py` is written so both may call the same logic; the API re-resolves every cluster id and re-runs entitlement, quota and co-residency before it will persist a placement (P.9). A cluster id from the browser is an assertion, never a fact.
11. **AI position in a governed workflow** — which actions an assistant may *never* take (answered by P3; confirm and record).
12. **Ownership of catalogue, rate cards and approval rules** — a permanent operational responsibility, not a project task.
13. **IDP re-visit** — ✅ **RESOLVED 2026-07-29: custom portal (now React + Carbon), Backstage declined** — adopting Backstage would mean re-architecting around its catalog/scaffolder/auth model and re-homing everything already built; conflicts with the simple/mainstream principle.
