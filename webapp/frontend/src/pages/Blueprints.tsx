...
const STATUS_TAG: Record<string, 'green' | 'blue' | 'red' | 'gray' | 'magenta' | 'purple'> = {
  ...
  stale: 'purple',      // certified once and the proof aged out (C2). Nothing
                        // failed — re-prove it rather than investigate it.
          ...
          <tbody>
            {rows.map((b) => (
              <tr key={`${b.technology_code}:${b.deployment_target}`}
                ...
                <td style={td}>
                  {(b.state === 'draft' || b.state === 'suspended' || b.state === 'stale') ? (
                    <TextInput
                ...
                <td style={td}>
                  ...
                  {(b.state === 'suspended' || b.state === 'stale') && b.notes && (
