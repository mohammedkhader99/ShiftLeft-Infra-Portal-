...
- **E2 — Governance depth:** **✅ F-GOV-01 approval SLA + breach escalation delivered** (server-computed on-time/due-soon/breached vs `APPROVAL_SLA_HOURS`; poller escalates a breach once via a Jira comment + `sla.breached` audit; My Requests SLA tag + Estate "approvals breaching SLA" KPI). Remaining: F-GOV-03/02/06/05/08/10 (policy-as-code, quorum, change windows, waivers, **four-eyes on the Jira approver [SoD's counterpart]**, evidence pack), F-ORC-02 (plan preview, if not already in 1.8).
