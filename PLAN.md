...
- **E2 — Governance depth:** **✅ F-GOV-01 approval SLA + breach escalation** (server-computed on-time/due-soon/breached vs `APPROVAL_SLA_HOURS`; poller escalates once via a Jira comment + `sla.breached` audit; My Requests SLA tag + Estate KPI). **✅ F-GOV-08 four-eyes** (the Jira approver must differ from the requester; live gate in the poller + approve, `four_eyes.blocked` audit + Jira comment, `FOUR_EYES_ENFORCED` default on, fail-open on unknown author). **✅ F-GOV-10 evidence pack** (`GET /api/requests/{ref}/evidence.pdf` — request + cost + approval + full audit trail + a re-verified "Audit chain: VERIFIED" attestation; My Requests download link). Remaining: F-GOV-03/02/06/05 (policy-as-code depth, quorum, change windows, waivers), F-ORC-02 (plan preview, if not already in 1.8).
