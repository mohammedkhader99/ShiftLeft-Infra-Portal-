...
**AI Assistant (8 capabilities)** — ✅ **request drafting from plain English (F-RPT-06)**, ✅ **cost explanation + optimization tips (F-RPT-07)**; ⬜ the rest (recommend sizing/cloud, detect gaps, predict time, compliance) → **E4**, **recommend-only per hard-rule P3** (never decides, executes, or holds credentials).
