...
- **E3 — FinOps & lifecycle:** **✅ E3.2 F-FIN-03 showback & chargeback** (`GET /api/showback?group_by=cost_centre|project|environment|owner&scope=active|committed` — sums the captured estimate by dimension; oversight-gated; Showback portal view with a Running/Committed toggle + proportion bars; reconciles with the estate's active monthly cost). Remaining: F-FIN-07 (TTL & renewal), F-FIN-02 (budget guardrails), F-FIN-01 (variance), F-FIN-06 (auto-shutdown), F-FIN-08 (quotas), F-LCM-08/09/10 (health, drift, ownership), F-LCM-03/06 (refresh, restore), F-IAM-07 + F-INT-05 (JIT access + vault delivery).
