...
| **8 — One component proven end to end** *(the evidence everything else lacked)* | ✅ **Done, 11 Aug 2026** | **Apache HTTP Server on OCI: requested in the portal, approved in Jira, provisioned, PROVEN SERVING, decommissioned, and confirmed gone.** The first entry in this log that carries no "unverified" caveat.<br><br>**REQ-2026-0100** — approval read from Jira `SDIMD-73532` at 06:50:29, signed handoff at 06:50:30, instance launched 06:51:32, `provisioned` at 06:52:55. **Two minutes twenty-five seconds** from a manager's approval to a running web server, with no infrastructure ticket and no manual step.<br><br>**The verification is the point.** Every previous claim of success meant "Terraform reported it created something". This one asked the machine. Via the Oracle Cloud Agent, on the instance itself:<br><br>`systemctl is-active httpd` → **active**<br>`ss -ltn` → **listening on \*:80 and \*:443**<br>`curl -s -o /dev/null -w '%{http_code}' http://localhost/` → **200**<br>`curl -s http://localhost/` → **the exact page the Terraform module wrote**<br><br>That distinction matters because three separate blueprints this week passed `terraform validate` and would have produced a healthy VM with no working service on it — Apache with the wrong OS image, Kafka with an unreachable archive, OKE with an attribute the provider does not accept. A clean plan proves a module compiles. Only the service answering proves it works.<br><br>**REQ-2026-0101** decommissioned it: approved 10:32:07, `Destroy complete! Resources: 1 destroyed.` at 10:33:39 — **92 seconds**. Verified in OCI rather than taken from the portal's word: the instance is `TERMINATED`, **the boot volume was deleted with it** (the classic orphan — OCI can preserve one and bill for it indefinitely), and the private IP was released, leaving the subnet with zero addresses in use.<br><br>**Every fix from the preceding week is load-bearing in that result:** the resource-kind derivation routing to the certified blueprint rather than a placeholder bucket; the image lookup selecting Oracle Linux instead of the shared Ubuntu default, without which `dnf`, `systemctl` and `firewall-cmd` would all have failed silently; the service gateway giving the private subnet a path to the package repositories; the subnet correction after the VCN was rebuilt; and the hostname read from the VNIC rather than recomputed.<br><br>*Incidental finding, unrelated to the portal:* two unattached 500 GB boot volumes (`aispoc2`, `aispoc2_new`) are billing in this compartment for nothing. They predate the portal and match no request.<br><br>**What this does and does not establish.** It establishes that the whole chain works for one component: catalogue → validation → policy → pricing → Jira approval → signed handoff → independent re-verification → Terraform → a working service → clean teardown. It does **not** establish that any other blueprint works. Managed PostgreSQL, Kafka and OKE remain unproven, and the honest measure at the time this row was written was **1 of 46 technologies verified end to end**. *(nginx and Redis were also unproven here; steps 9 and 10, later the same day, took the count to 3.)* |
| **9 — The generic blueprint proven** *(nginx and Redis 7)* | ✅ **Done, 11 Aug 2026** | The `service-vm` blueprint — one module serving many technologies, with the difference held as data — booted and checked. **nginx**: `nginx-1.20.1`, service active, listening on `0.0.0.0:80`, `curl localhost` → **200**. **Redis**: `redis-7.2.14`, service active, `redis-cli PONG`, set/get round-tripped, and bound to `127.0.0.1` only, as the profile intends.<br><br>**The boot test found two bugs, and the first is the reason this row exists.**<br><br>**`service-vm` had never worked, for anything.** Every diagnostic message contains a colon — `PORTAL: package install FAILED` — and a bare YAML scalar containing `': '` is a MAPPING. cloud-init received a dict where it expected a command, raised `Failed to shellify`, and abandoned the **entire** `runcmd` block. The failure took the worst available shape: `write_files` still ran, so `/etc/infra-portal-configured` appeared announcing the technology was configured, while nothing was installed and the VM booted healthy. Observed exactly that on the first nginx boot — marker present, no portal log, nginx inactive, `curl` returning 000. The blueprint had shipped four days earlier with passing tests, because those tests asserted on substrings of the rendered TEXT; nobody parsed it as YAML, which is what cloud-init does.<br><br>**`redis7` installed Redis 6.** Oracle Linux 9 filters modular packages until the stream is enabled, so `dnf install redis` gives 6.2.22 — installed, running, answering `PONG`, and a major version behind what the catalogue named. Profiles can now declare a module stream. Proven by hand on a VM first (7.2.14), then re-proven from the configuration the code *generates*, so the claim rests on evidence rather than inference.<br><br>Both are now in `configure.VERIFIED_CODES` with the evidence recorded beside them. `apache` is deliberately excluded: the certified Apache blueprint renders its own Terraform template and never calls this module, so step 8 proved that template, not this one.<br><br>Six VMs, all destroyed. |
| **10 — A multi-component request proven** | ✅ **Done, 11 Aug 2026** | **REQ-2026-0107**: one request for Apache, nginx and Redis produced two VMs — Apache alone on one, nginx and Redis together on the other — each running only its own software, then decommissioned by REQ-2026-0108 with both resources torn down. The stack path now works in both directions.<br><br>**The first attempt (REQ-2026-0105) failed, and finding out required asking the machines.** The first-boot script was rendered once from the whole request and handed to *every* resource, so the service-vm was told to install Apache as well. httpd and nginx both want port 80; httpd started first and took it, and **nginx failed to start**. The portal reported the request provisioned, Jira moved to Resolved, and a third of what was asked for was dead:<br><br>`service-vm: nginx FAILED · redis active · httpd active · port 80 answering 403`<br><br>Each resource is now told to install only what its blueprint declares it builds — data the manifests already carried, so no contract change was needed. The same filter scopes the ports opened, so a VM cannot open a port for software it does not run. Re-run as REQ-2026-0107 and confirmed on both machines: `rpm -q httpd` on the service-vm returns **not installed**, nginx is **active**, and port 80 answers **200 from nginx** rather than 403 from httpd.<br><br>**A note on the certified set.** It now contains exactly `apache`, `nginx`, `redis7` — the three technologies verified end to end. Certification was withdrawn from those that had never been booted, so *certified* and *proven* describe the same set. Keeping that true is what makes the badge worth anything.<br><br>**Measured state: 3 of 46 technologies certified, and all 3 verified end to end** — against the 4 certified and 0 verified this document recorded on 4 Aug 2026 (step 6). |
