    ...
    profile = {
        ...
        "container": {
            ...
            **({"platform_digest": image["platform_digest"]}
            ...
            # AND WHAT THE CONTAINER ITSELF ASKED FOR, when a machine has
            # already told us. `library/mysql` exits without being told how
            # to set its root password, and declares nothing about it: the
            # only source is the log of a machine that tried. What may be
            # supplied is decided in api/container_env, which chooses a
            # password the MACHINE generates and never one this portal holds.
            **({"environment": {**(image.get("environment") or {}),
                                **({"ACCEPT_EULA": "Y"} if licence_accepted() else {})}}
               if (image.get("environment") or licence_accepted()) else {}),
        },
        ...
        "_note": (
            ...
            + (f" The container asked for "
               ...
               f"its own log." if image.get("environment") else "")
