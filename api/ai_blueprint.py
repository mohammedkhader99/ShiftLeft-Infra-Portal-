    ...
    profile = {
        ...
        "container": {
            ...
            **({"platform_digest": image["platform_digest"]}
            ...
            # password and declares nothing about one; SQL Server exits without
            # ACCEPT_EULA=Y and does not declare that either. The only source
            # for both is the log of a machine that tried, which the boot report
            # captures when a service never comes up. What may be supplied is
            # decided in api/container_env: a password the MACHINE generates and
            # never one this portal holds, and a licence only where an operator
            # has recorded the decision in the Admin console.
            #
            # ACCEPT_EULA USED TO BE SET ON EVERY IMAGE, deliberately: a
            # container that does not read it ignores it, one that needs it and
            # does not get it dies, and a hand-curated list of which images care
            # is what every earlier version of this system got wrong. That was
            # the right call while there was no third option; C9 built one, and
            # the blanket's cost showed the day MySQL was certified. MySQL is
            # GPL, never reads the variable, and its recipe carried
            # ACCEPT_EULA=Y under a note saying an operator had accepted vendor
            # licence terms -- a licence claim on a technology that needs no
            # licence, in an artefact an approver reads. An image that truly
            # needs it now fails once, says so in its own log, and is answered
            # on the retry.
            # `dict(...)` is defensive and DELIBERATELY UNTESTED: the profile
            # is serialised to JSON before this function returns, so no live
            # reference escapes and sharing the caller's dict changes nothing
            # observable. A plant proved that -- removing the copy broke no
            # test, and the test written to catch it could not fail. Kept
        ...
        "_note": (
            ...
            # WHAT WAS SUPPLIED AND WHY, naming only what the container asked
            # for. This used to announce a recorded licence acceptance whenever
            # the setting was on, whether or not the image had ever mentioned a
            # licence -- so MySQL's recipe claimed one.
            + (f" The container asked for "
               f"{', '.join(sorted(image['environment']))} and was given it; "
               f"any password is generated by the machine and printed once in "
               f"its own log"
               + (", and an operator has recorded acceptance of the vendor's "
                  "licence terms in the Admin console"
                  if "ACCEPT_EULA" in image["environment"] else "")
               + "." if image.get("environment") else "")
