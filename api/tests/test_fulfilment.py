...
def test_database_on_oci_is_manual(session):
    # The headline case: requesting Postgres does NOT create a Postgres.
...
def test_most_of_the_catalogue_is_manual_on_oci(session):
    ...
    # Exactly the three compute technologies + OCI object storage.
    assert {t.code for t in automated} == {"compute-vm", "rhel9", "win2019", "oci-objectstorage"}
...
def test_automated_targets_filters_the_list(session):
    ...
    assert fulfilment.automated_targets(pg, ["onprem", "oci", "aws"]) == []
...
def test_classifier_agrees_with_the_provisioners_resource_kind(session):
    ...
    for code in ("compute-vm", "rhel9", "win2019", "oci-objectstorage", "postgres16", "kafka"):
        ...
        if tech.resource_kind == "oci-instance":
...
def test_lookups_exposes_automated_targets(client):
    ...
    # The catalogue's majority: available on five targets, automated on none.
